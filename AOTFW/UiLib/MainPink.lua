...
if getgenv().Connections and getgenv().Connections["ClearConnectionsFunction"] then
	getgenv().Connections["ClearConnectionsFunction"]()
end
...
do -- Load items
	...
	TabButton.Font = Enum.Font.Merriweather
	TabButton.Text = "Menu"
	...
	Tabs.Name = "Tabs"
	Tabs.Parent = Main
	Tabs.BackgroundColor3 = Color3.fromRGB(90, 50, 80)
	...
	Inner.ImageColor3 = Color3.fromRGB(124, 53, 70)
	...
	Value.Font = Enum.Font.Merriweather
	Value.Text = "6.00"
	...
	Text_2.Font = Enum.Font.Merriweather
	Text_2.Text = "Slider"
	...
	UIGradient.Color = ColorSequence.new({
		ColorSequenceKeypoint.new(0.00, Color3.fromRGB(0, 129, 167)),
		ColorSequenceKeypoint.new(1.00, Color3.fromRGB(0, 160, 168)),
	})
	...
	heading.ZIndex = 3
	heading.Font = Enum.Font.Merriweather
	...
	SubHeading.ZIndex = 3
	SubHeading.Font = Enum.Font.Merriweather
	...
	Text_3.Font = Enum.Font.Merriweather
	...
	DropdownOption.Font = Enum.Font.Merriweather
	...
	Title_3.ZIndex = 4
	Title_3.Font = Enum.Font.Merriweather
	...
	Inner_2.ImageColor3 = Color3.fromRGB(124, 53, 70)
	...
	Selected.Font = Enum.Font.Merriweather
	...
	Title_4.ZIndex = 4
	Title_4.Font = Enum.Font.Merriweather
	...
	Inner_3.ImageColor3 = Color3.fromRGB(124, 53, 70)
	...
	Value_2.Font = Enum.Font.Merriweather
	...
	Text_4.Font = Enum.Font.Merriweather
	...
	new = function(options)
		...
		options = settings
			.new({
				...
				shadow = 10,
				transparency = 0.2,
				color = Color3.fromRGB(255, 133, 162),
				boardcolor = Color3.fromRGB(54, 36, 52),
				rounding = 5,
				animation = 0.1,
			...
			do
				...
				function types.switch(switchOptions)
					...
					function self.set(boolean)
						if (not not boolean) == self.on then
							return
						end
						...
						spawn(function()
							if inside and findBrowsingTopMost() == main then
								while mouse.held do
									...
									local p = getMouse()
									local x =
										math.floor(math.clamp(p.X - inner.AbsolutePosition.X, 0, sliderOptions.size))
									local m = sliderOptions.size / math.abs(sliderOptions.max - sliderOptions.min)
									local v = math.floor(x / m)
									self.set(
										v
											+ (
												sliderOptions.min < sliderOptions.max and sliderOptions.min
												or sliderOptions.max
											)
									)
									RunService.Heartbeat:Wait()
								end
				...
				function types.color(colorOptions)
					...
					button.MouseButton1Click:Connect(function()
						...
						if self.visible then
							colorPicker.Position = UDim2.new(
								0,
								colorPickerButton.AbsolutePosition.X + colorPickerButton.AbsoluteSize.X,
								0,
					...
					function self.close()
						...
						resize(
							colorPicker:FindFirstChild("Content"),
					...
					function self.open()
						...
						resize(
							colorPicker:FindFirstChild("Content"),
							{ Size = UDim2.new(1, 0, 0, colorCache.content_size) },
							options.animation
						)
					end
					...
					colorCache.update_layers(
						colorPicker.AbsoluteSize.Y + colorPicker:FindFirstChild("Content").AbsoluteSize.Y
					)

					colorPicker:FindFirstChild("Content"):GetPropertyChangedSignal("Size"):Connect(function()
						colorCache.update_layers(
							main.AbsoluteSize.Y + colorPicker:FindFirstChild("Content").AbsoluteSize.Y
						)
					end)
							...
							if Entered1 and findBrowsingTopMost() == colorPicker then
								spawn(function()
									...
									while mouse.held do -- palette
										local p = getMouse()
										local x1 =
											math.clamp(p.X - palette.AbsolutePosition.X, 0, palette.AbsoluteSize.X)
										local v1 = x1 / palette.AbsoluteSize.X
										local x2 =
											math.clamp(p.Y - palette.AbsolutePosition.Y, 0, palette.AbsoluteSize.Y)
										local v2 = x2 / palette.AbsoluteSize.Y
										h = 1 - v1
										s = 1 - v2

										local sv1 =
											math.clamp(v1, 0, (palette.AbsoluteSize.X - 6) / palette.AbsoluteSize.X)
										local sv2 =
											math.clamp(v2, 0, (palette.AbsoluteSize.Y - 6) / palette.AbsoluteSize.Y)
										resize(
											paletteIndicator,
											{ Position = UDim2.new(sv1, 0, sv2, 0) },
											options.animation
										)
							...
							if Entered2 and findBrowsingTopMost() == colorPicker then
								spawn(function()
									...
									while mouse.held do -- saturation
										local p = getMouse()
										local x1 = math.clamp(
											p.Y - saturation.AbsolutePosition.Y,
											0,
										...
										local sv1 =
											math.clamp(v1, 0, (palette.AbsoluteSize.Y - 2) / palette.AbsoluteSize.Y)
										resize(
											saturationIndicator,
											{ Position = UDim2.new(0, 0, sv1, 0) },
											options.animation
										)
				...
				function types.dropdown(dropdownOptions)
					...
					dropdownOptions = settings
						.new({
							text = "New Dropdown",
							size = 300,
							color = Color3.fromRGB(124, 53, 70),
							rounding = options.rounding,
							selectioncolor = Color3.fromRGB(124, 53, 70),
						})
					...
					inner.MouseButton1Click:Connect(function()
						...
						if self.visible then
							dropdownWindow.Position = UDim2.new(
								0,
								dropdownButton.AbsolutePosition.X + dropdownButton.AbsoluteSize.X,
								0,
								dropdownButton.AbsolutePosition.Y
							)
							self.open()
					...
					function self.close()
						...
						resize(
							dropdownWindow:FindFirstChild("Content"),
							{ Size = UDim2.new(1, 0, 0, 0) },
							options.animation
						)
					end
					...
					function self.open()
						...
						resize(
							dropdownWindow:FindFirstChild("Content"),
							{ Size = UDim2.new(1, 0, 0, dropdownCache.content_size) },
							options.animation
						)
					end

					function dropdownCache.update_layers(y)
						dropdownWindow:FindFirstChild("Shadow").Position =
							UDim2.new(0, options.shadow, 0, options.shadow)
					...
					dropdownCache.update_layers(
						dropdownWindow.AbsoluteSize.Y + dropdownWindow:FindFirstChild("Content").AbsoluteSize.Y
					)

					dropdownWindow:FindFirstChild("Content"):GetPropertyChangedSignal("Size"):Connect(function()
						dropdownCache.update_layers(
							main.AbsoluteSize.Y + dropdownWindow:FindFirstChild("Content").AbsoluteSize.Y
						)
					end)
					...
					function self.new(name)
						...
						assert(
							rawget(dropdownObjects, name) == nil,
							string.format("object already exists in dropdown '%s'", dropdownOptions.text)
						)
					...
					do -- search bar
						local TextBox = dropdownWindow
							:FindFirstChild("Content")
							:FindFirstChild("Search")
							:FindFirstChild("Outer")
							:FindFirstChild("Inner")
							:FindFirstChild("TextBox")
				...
				function types.folder(folderOptions)
					...
					function self.open()
						...
						resize(
							folder,
							{ Size = UDim2.new(1, 0, 0, 20 + countSize(folderItems).Y + 2) },
							folderOptions.animation
						)
